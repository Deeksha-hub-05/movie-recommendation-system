...
if not TMDB_API_KEY:
    raise RuntimeError("TMDB_API_KEY is missing.Put it in your .env file as TMDB_API_KEY=your_key")
...
async def tmdb_get(path: str, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    headers = {
        ...
        "User-Agent": "movie-recommender-api"
    }
    ...
    try:
        ...
        raise HTTPException(
            status_code=504,
            detail="TMDB connection timeout"
        )

    except httpx.ReadTimeout:
        raise HTTPException(
            status_code=504,
            detail="TMDB read timeout"
        )
...
def build_title_to_idx_map(indices:any) ->Dict[str,int]:
    ...
    try:
        ...
        raise RuntimeError(
            "indice.pkl must be dict or pandas Series-like(with .items())"
        )
...
def get_local_idx_by_title(title:str)->int:
    ...
    raise HTTPException(
        status_code=404,
        detail=f"Movie titled not found in local dataset:  '{title}'"
    )
...
#Details+TF-IDF Recommendation + Genre Recommendation
